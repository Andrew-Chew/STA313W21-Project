  ...
  shinyApp(
    ...
    server = function(input, output, session) {
      ...
      selected_data <- reactive({
        ...
        columns <- c("year")
        columns <- if (input$country != "ALL") c("country", columns) 
        columns <- if (input$view == 1) c(columns, "cumulative_co2") else c(columns, "cumulative_gas_co2")

        if (input$country == "All" & input$view == 1) {
          dataset[, c("year", "cumulative_co2")] %>%
            group_by(year) %>%
            filter(year >= input$years[1] & year <= input$years[2]) %>%
            summarise(cum = sum(cumulative_co2, na.rm = TRUE))
        }
        else if (input$country == "All" & input$view == 2) {
          dataset[, c("year", "cumulative_gas_co2")] %>%
            group_by(year) %>%
            filter(year >= input$years[1] & year <= input$years[2]) %>%
            summarise(cum = sum(cumulative_gas_co2, na.rm = TRUE))
        }
        else if (input$country != "All" & input$view == 1) {
          dataset[, columns] %>%
