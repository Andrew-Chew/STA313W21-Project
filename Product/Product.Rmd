...
p2_emissions_clean <- p2_emissions_data[c("country", "year", "cumulative_co2", "cumulative_gas_co2")]
  ...
  shinyApp(
    ui = fluidPage(
      fluidRow(
        ...
        column(6, plotOutput("co2emissions")),
    ...
    server = function(input, output, session) {
      ...
      selected_emissions_data <- reactive({
        if (input$view == 1) {
            emissions_data[, c("country", "year", "cumulative_co2")] %>%
            ...
            emissions_data[, c("country", "year", "cumulative_gas_co2")] %>%
            filter(country == input$country) %>%
            group_by(year) %>%
            filter(year >= input$years[1] & year <= input$years[2]) %>%
      ...
      output$co2emissions <- renderPlot({
        ggplot(selected_emissions_data(), aes(x = year, y = cum)) +
          geom_point() +
          geom_path() +
          geom_smooth() +
          theme_wsj()
