...
<!-- <<<<<<< HEAD -->
|
|       What is climate change? According to [NASA](https://climate.nasa.gov/resources/global-warming-vs-climate-change/), climate change is a long-term change in the average weather patterns that have come to define Earth’s local, regional and global climates. These changes have a broad range of observed effects that are synonymous with the term.Changes observed in Earth’s climate since the early 20th century are primarily driven by human activities, particularly fossil fuel burning, which increases heat-trapping greenhouse gas levels in Earth’s atmosphere, raising Earth’s average surface temperature. These human-produced temperature increases are commonly referred to as global warming. Natural processes can also contribute to climate change, including internal variability (e.g., cyclical ocean patterns like El Niño, La Niña and the Pacific Decadal Oscillation) and external forcings (e.g., volcanic activity, changes in the Sun’s energy output, variations in Earth’s orbit). A significant proportion of these emissions comes from just transportation!

```{r echo=FALSE, figures-side, fig.show="hold", fig.align="center", out.width="50%"}
par(mar = c(4, 4, .1, .1))
...
transport_pie <- highchart () %>% 
  ...
  hc_title(
    text = "Proportion of CO2 Emissions by Transportation Types in 2016"
    ) 
...
|       Many people own cars in Canada. Did you know that the average Canadian vehicle, which burns 2 000 L of gasoline every year, releases about 4 600 kg of CO2 into the atmosphere according to [Natural Resources Canada](https://www.nrcan.gc.ca/sites/www.nrcan.gc.ca/files/oee/pdf/transportation/fuel-efficient-technologies/autosmart_factsheet_6_e.pdf)? This number might not mean much to you. So let's see how many trees are working to absorb our emissions. According to [US Department of Agriculture](https://www.usda.gov/media/blog/2015/03/17/power-one-tree-very-air-we-breathe), in one year a mature tree will absorb more than 48 pounds (roughly 22 kg) of CO2 from the atmosphere and release oxygen in exchange.  

![](www/treesncar.png)

|       209 trees work to absorb just 1 average vehicle's CO2 emissions. Globally we produced 5.7 billion tonnes of CO2 from only transportation in 2016. To absorb this from the atmosphere, we need 259 billion trees. World's largest forest Amazon rain forest roughly have 390 billion trees according to [EurekAlert!](https://www.eurekalert.org/pub_releases/2013-10/fm-fms101413.php). We would need about 66% of the trees in Amazon to absorb the CO2 emissions produced only from transportation. 

<center>
![Amazon forest on the world map](www/map_amazon.png)
</center>
<center>
*image sourced from [Ocean Futures](http://www.oceanfutures.org/exploration/expeditions/amazon)
</center>
...
## How can you help?
|
|       Now we understand what climate change is and how much CO2 emission we contribute to the environment through transportation everyday. As a responsible inhabitant to this planet, lets try to think of ways to reduce the level of air pollution we cause to the environment!

|
|       According to a study by [Clean Energy Canada](https://www.equiterre.org/sites/fichiers/fmm_transportation_recs.pdf) in 2016, about 15.4 million people commute regularly. Amongst the commuters, 74% of them choose to commute by private automobile. However, is commuting by private vehicle considered **Environmental Friendly**? Is there an alternative to driving that would cause less harm to our environment? If everyone in Canada decide to make a small change to their daily commuting habit, after multiplying this reduction of CO^2^ by 15.4 million times, it will make a **huge** difference and impact positively to our environment.

|
|       In the section below, we provide an interactive activity that allows you to compare the CO^2^ emission per kilometer of any two vehicles of your choice. The graphic shows **an approximation** of CO^2^ emission ratio between the two vehicles, presented by number of people standing in front. The less people standing in front of the vehicle type, the **more selfish** the vehicle is! (more CO^2^ is produce by that type of vehicle with less people using it). Underneath the graphic, we also calculate the exact ratio for you and summarize the result to the vehicles you pick. Find out if you are **responsible** for the global warming and explore any possible alternatives that are better than your current commuting tools.
|
|
...
ui <- fluidPage(
  ...
  fluidRow(
    # Print text at the end of this section to provide accurate comparison
    column(12, align = "center",
    textOutput(outputId = "word"),
    tags$head(tags$style("#word{color: black;
                                 font-size: 20px;
                                 font-style: italic;
                                 }"
                         )))
...
server <- function(input, output, session){
  ...
  # list the conditions for showing the images
  output$myImage <- renderImage({
    if (compare(temp_input(), c(1,1))){
      return(list(
        src = "www/f vs f.png",
        fileType = "image/png",
        alt = "FlightFlight"
      ))
    } else if (compare(temp_input(), c(2,2))){
      return(list(
        src = "www/car_vs_car.png",
        fileType = "image/png",
        alt = "CarCar"
      ))
    } else if (compare(temp_input(), c(3,3))){
      return(list(
        src = "www/train_vs_train.png",
        fileType = "image/png",
        alt = "TrainTrain"
      ))
    } else if (compare(temp_input(), c(4,4))){
      return(list(
        src = "www/EV_vs_EV.png",
        fileType = "image/png",
        alt = "EVEV"
      ))
    } else if (compare(temp_input(), c(5,5))){
      return(list(
        src = "www/bus_vs_bus.png",
        fileType = "image/png",
        alt = "busbus"
      ))
    } else if (compare(temp_input(), c(1,2))| compare(temp_input(), c(2,1))){
      return(list(
        src = "www/car_vs_flight.png",
        fileType = "image/png",
        alt = "carflight"
      ))
    } else if (compare(temp_input(), c(1,3))| compare(temp_input() ,c(3,1))){
      return(list(
        src = "www/train_vs_flight.png",
        fileType = "image/png",
        alt = "trainflight"
      ))
    } else if (compare(temp_input(), c(1,4))| compare(temp_input(), c(4,1))){
      return(list(
        src = "www/EV_vs_flight.png",
        fileType = "image/png",
        alt = "flightEV"
      ))
    } else if (compare(temp_input() , c(2,3))|compare(temp_input(), c(3,2))){
      return(list(
        src = "www/car_vs_train.png",
        fileType = "image/png",
        alt = "cartrain"
      ))
    } else if (compare(temp_input(), c(2,4)) | compare(temp_input() , c(4,2))){
      return(list(
        src = "www/car_vs_EV.png",
        fileType = "image/png",
        alt = "carEV"
      ))
    } else if (compare(temp_input() ,c(3,4)) | compare(temp_input(),c(4,3))){
      return(list(
    ...
    } else if (compare(temp_input() , c(1,5))| compare(temp_input(), c(5,1))){
      return(list(
        src = "www/bus_vs_flight.png",
        fileType = "image/png",
        alt = "busflight"
      ))
    } else if (compare(temp_input() , c(2,5))| compare(temp_input(), c(5,2))){
      return(list(
        src = "www/bus_vs_car.png",
        fileType = "image/png",
        alt = "buscar"
      ))
    } else if (compare(temp_input(), c(3,5))|compare(temp_input(), c(5,3))){
      return(list(
        src = "www/bus_vs_train.png",
        fileType = "image/png",
        alt = "bustrain"
      ))
    } else if (compare(temp_input() ,c(4,5))| compare(temp_input(), c(5,4))){
      return(list(
  ...
  # Calculate the ratio between the two vehicles 
  calculation <- reactive({
    if (compare(temp_input(), c(1,1)) | compare(temp_input(), c(2,2)) | compare(temp_input(), c(3,3)) |compare(temp_input(), c(4,4)) | compare(temp_input(), c(5,5))){
      return(1)
    } else if (compare(temp_input(), c(1,2))| compare(temp_input(), c(2,1))){
      return(1.23)
    } else if (compare(temp_input(), c(1,3))| compare(temp_input() ,c(3,1))){
  ...
  # Vehicle Text Generator
  vtype <- function(number){
    if (number == 1){
      return("airplane")
    } else if (number == 2){
      ...
      return("electric vehcle")
    } else if (number == 5){
      return("bus")
    }
  }

  # The word description of the comparison
  output$word <- renderText({
    if (compare(temp_input(), c(1,1)) | compare(temp_input(), c(2,2)) | compare(temp_input(), c(3,3)) |compare(temp_input(), c(4,4)) | compare(temp_input(), c(5,5))){
      paste("One passenger taking", vtype(input$first_vehicle), "has the same level of CO2 emission/km as", calculation(), "person taking", vtype(input$second_vehicle))
    } else if (compare(temp_input(), c(1,2))| compare(temp_input(), c(2,1))){
      paste("One passenger taking a car has the same level of CO2 emission/km as", calculation(), "passengers taking an airplane flight." )
    } else if (compare(temp_input(), c(1,3))| compare(temp_input() ,c(3,1))){
      paste("One passenger taking the airplane has the same level of CO2 emission/km as", calculation(), "passengers taking the train.")
    } else if (compare(temp_input(), c(1,4))| compare(temp_input(), c(4,1))){
      paste("One passenger taking the airplane has the same level of CO2 emission/km as", calculation(), "passengers taking/driving Electric Vehicles. ")
    } else if (compare(temp_input() , c(2,3))|compare(temp_input(), c(3,2))){
      paste ("One person driving a car has the same level of CO2 emission/km as", calculation(), "passengers taking the train.")
    } else if (compare(temp_input(), c(2,4)) | compare(temp_input() , c(4,2))){
      paste("One person driving a car has the same level of CO2 emission/km as", 
            calculation(), "people taking/driving Electric Vehicles. ")
    } else if (compare(temp_input() ,c(3,4)) | compare(temp_input(),c(4,3))){
      paste("One person taking/driving an Electric Vehicle has the same CO2 emission/km as", calculation(), "people taking the train.")
    } else if (compare(temp_input() , c(1,5))| compare(temp_input(), c(5,1))){
      paste("One person taking an airplane has the same CO2 emission/km as", calculation(), "people taking the bus.")
    } else if (compare(temp_input() , c(2,5))| compare(temp_input(), c(5,2))){
      paste("One person taking/driving a car has the same amount of CO2 emission/km as", calculation(), "people taking the bus.")
    } else if (compare(temp_input(), c(3,5))|compare(temp_input(), c(5,3))){
      paste("One person taking the train has the same amount of CO2 emission/km as", calculation(), "people taking the bus.")
    } else if (compare(temp_input() ,c(4,5))| compare(temp_input(), c(5,4))){
      paste("One person taking/driving an Electrical Vehicle has the same amount of CO2 emission/km as", calculation(), "people taking the bus.")
    }
  })
...
shinyApp(ui, server, options = list(height = 600, width = 1000)) # Change this to resize widget.

```

|
|       After some exploration, you probably noticed that driving a personal vehicle is not the best way to commute considering its high CO^2^ emission level. You should also noticed that all public transportation, excluding airplane, are more environmentally friendly comparing to traditional personal vehicles. In this particular activity, we only selected a few transportation types to include into our little vehicle comparison, however, there are many other ways to commute and make less carbon emission into the air. For example- riding a bicycle! Vehicles like bicycles are the ideal commuting tools that doesn't cause air pollution during the commute, however they also face serious geographic constraints that only allow short distance travels. Thus, the lesson here is to avoid using personal vehicle and take advantage of the public transportation as much as possible. In situation when we have to drive a car, act **selflessly** by sharing rides with others so we contribute less Co^2^ to our environment per person.
